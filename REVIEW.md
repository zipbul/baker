...
| 단계 | 우선순위 | 대상 항목 | 기준 |
|------|----------|-----------|------|
| **1단계: 즉시** | 🔴 Critical | C-1, C-2, C-5, C-7, C-8, C-9 | 런타임에 잘못된 결과를 반환하거나 기능 손실을 유발하는 실제 버그 |
| **2단계: 곧** | 🟠 High | C-3, C-6, C-10, C-11, C-12, B-6, B-1, B-3, B-7, B-9 | 스펙 위반, 안전장치 누락, 잠재적 크래시, API 의미 불일치 |
| **3단계: 안정화** | 🟡 Medium | B-2, B-4, B-8, B-11, C-13, C-14, C-15, D-1, D-4 | Silent failure 제거 + 버그 원인이 된 코드 중복 제거 |
| **4단계: 개선** | 🟢 Low | A-1, B-10, D-2, D-3, D-5, D-6, F-1~F-7, B-5, C-16, C-17 | 설계 개선, 리팩토링, 코드 품질 |
...
### A-1. `enableCircularCheck` 옵션 제거 `🟢 Low` `⚠️ Breaking`
...
### B-1. `deserialize-builder.ts:499-503` — type fn() 에러 완전 삼킴 `🟠 High`
...
### B-3. `deserialize-builder.ts:414-416` — 미지원 변환 타입 침묵 `🟠 High`
...
### B-4. `field.ts:228-229` — `transformDirection` 검증 없음 `🟡 Medium`
...
### B-6. `constructor?.name === 'AsyncFunction'` — 취약한 async 감지 `🟠 High`
...
### B-7. `circular-analyzer.ts:34-38` — `meta.type.fn()` try-catch 없음 `🟠 High`
...
### B-8. `collect.ts:73-77` — schema merge 배열 입력 미차단 `🟡 Medium`
...
### B-9. `seal.ts:142-143` — seal 에러에 클래스명 없음 `🟠 High`
...
### B-10. `configure.ts:27` — seal 후 configure() 호출 처리 `🟢 Low` `⚠️ Breaking`
...
### B-11. `to-json-schema.ts:392-394` — `applyNullable` null 중복 추가 `🟡 Medium`
...
### C-1. `seal.ts:38-47` — `analyzeAsync` discriminator visited Set 미공유 `🔴 Critical`
...
### C-2. `deserialize-builder.ts:666,672` — Set/Map stopAtFirstError 에러 경로 인덱스 누락 `🔴 Critical`
...
### C-3. `to-json-schema.ts:326-337` — discriminator JSON Schema 구조 위반 `🟠 High`
...
### C-5. `rules/number.ts:84` — `isDivisibleBy(0)` 침묵 실패 `🔴 Critical`
...
### C-6. `rules/string.ts:381` — `isURL` 포트 65536-99999 허용 `🟠 High`
...
### C-7. `rules/typechecker.ts:40` — `isNumber maxDecimalPlaces` 과학 표기법 파싱 오류 `🔴 Critical`
...
### C-8. `serialize-builder.ts:123` — serialize에서 discriminator 완전 무시 `🔴 Critical`
...
### C-9. `to-json-schema.ts:362` — nullable `$ref`가 invalid JSON Schema 생성 `🔴 Critical`
...
### C-10. `configure.ts:34` — `stripUnknown` API 이름이 실제 동작과 불일치 `🟠 High`
...
### C-11. `serialize-builder.ts:136-138` — nested array serialize 시 null 요소 크래시 `🟠 High`
...
### C-12. `number.ts:14,37` — `min(NaN)`, `max(NaN)` 침묵 통과 `🟠 High`
...
### C-13. `seal.ts:30` — `analyzeAsync`에서 `meta.type.fn()` try-catch 없음 `🟡 Medium`
...
### C-14. `to-json-schema.ts:146-149` — class-level `@Schema`가 `$defs`/`properties` 덮어쓰기 `🟡 Medium`
...
### C-15. `to-json-schema.ts:413` — user composition schema가 auto schema를 완전 제거 `🟡 Medium`
...
### C-17. `deserialize-builder.ts:741-742` — nested array 레벨 룰에 커스텀 message/context 누락 `🟢 Low`
...
### D-1. `buildRulesCode()` 254줄 god function 분해 `🟡 Medium`
...
### D-4. Array/Set/Map each 코드 중복 제거 `🟡 Medium`
...
### ~~F-3. `_sealOnDemand` snapshot 최적화~~ `삭제`
...
### F-7. serialize/deserialize 삼중 SEALED 체크 단순화 `🟢 Low`
