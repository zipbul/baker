...
import type { RawClassMeta, RawPropertyMeta, TransformDef, TypeDef } from '../metadata';
...
class SerializeBuilder<T> {
  ...
  private generateFieldCode(fieldKey: string, meta: RawPropertyMeta): string {
    ...
    if (type?.collection) {
      const innerCode = this.generateCollectionCode(
        fieldKey,
        type,
        type.collection,
        outputKey,
    ...
    if (type && (type.resolvedClass || type.discriminator || (type.fn !== undefined && meta.flags.validateNested))) {
      innerCode = this.generateNestedCode(fieldKey, meta, type, outputKey, sk, fieldVal, useOptionalGuard, serTransforms);
    } else {
  ...
  /** Set/Map collection serialize: Set → Array, Map → plain object (string-keyed only). */
  private generateCollectionCode(
    ...
    sk: string,
    fieldVal: string,
    useOptionalGuard: boolean | undefined,
    ...
    if (useOptionalGuard) {
      return `if (${fieldVal} !== undefined && ${fieldVal} !== null) {\n  ${nestedCode}\n} else if (${fieldVal} === null) {\n  ${outputTarget} = null;\n}\n`;
    }
    return `if (${fieldVal} != null) {\n  ${nestedCode}\n} else {\n  ${outputTarget} = ${fieldVal};\n}\n`;
  }

  /** Nested-DTO serialize — @Type/@ValidateNested field, either a discriminated union (delegates to
   *  generateDiscriminatorCode) or a single resolved class, single- or each-array mode. */
  private generateNestedCode(
    ...
    if (type.discriminator) {
      nestedCode = this.generateDiscriminatorCode(fieldKey, type, hasEach, sk, fieldVal, outputTarget);
    } else {
  ...
  /** Discriminator serialize — instanceof dispatch to the most-specific matching subtype's executor,
   *  optionally re-stamping the discriminator property; single- or each-array mode. */
  private generateDiscriminatorCode(
    fieldKey: string,
    type: TypeDef,
    hasEach: boolean,
    sk: string,
 ...
 * Public function entry point (the shape seal.ts calls): instantiates SerializeBuilder and
 * returns its built executor.
 */
