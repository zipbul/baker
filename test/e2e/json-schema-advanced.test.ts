...
describe('toJsonSchema — whitelist option', () => {
  ...
  it('whitelist not set → no unevaluatedProperties', () => {
...
describe('toJsonSchema — isNotEmptyObject → minProperties', () => {
  ...
  it('minProperties: 1 mapping', () => {
    const schema = toJsonSchema(MinPropDto);
    expect(schema.properties!.config!.minProperties).toBe(1);
  });
});

describe('toJsonSchema — required fields', () => {
  it('required fields → included in required array', () => {
    const schema = toJsonSchema(WhitelistSchemaDto);
...
describe('toJsonSchema — direction-specific schema differences', () => {
  it('direction: deserialize → deserializeOnly @Expose name used', () => {
    ...
    expect(schema.properties!['secret']).toBeDefined(); // serializeOnly Exclude → visible in deserialize
  });

  it('direction: serialize → serializeOnly @Expose name used', () => {
    ...
    expect(schema.properties!['secret']).toBeUndefined(); // serializeOnly Exclude → not visible in serialize
  });
});
