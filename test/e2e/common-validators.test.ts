...
import {
  ...
  Equals, NotEquals, IsIn, IsNotIn, IsEmpty, IsNotEmpty, IsEnum,
  IsString, IsNumber,
} from '../../index';
...
class EqualsDto { @Equals('yes') answer!: string; }
class NotEqualsDto { @NotEquals('no') answer!: string; }
class IsInDto { @IsIn(['a', 'b', 'c']) choice!: string; }
class IsNotInDto { @IsNotIn([1, 2, 3]) val!: number; }
class IsEmptyDto { @IsEmpty() field!: unknown; }
class IsNotEmptyDto { @IsNotEmpty() field!: unknown; }

enum Color { Red = 'red', Green = 'green', Blue = 'blue' }
class EnumDto { @IsEnum(Color) color!: Color; }
