...
import { isBakerIssueSet, Field, Baker } from '../../index';
...
describe('error — integration', () => {
  it('should return BakerIssueSet on invalid input', async () => {
    const result = await baker.deserialize(ErrorDto, { name: 123, age: 25, email: 'x@y.com' });
  ...
  it('BakerIssueSet.errors should include path and code', async () => {
    const result = await baker.deserialize(ErrorDto, { age: 25, email: 'x@y.com' });
    assertBakerIssueSet(result);
    expect(result.errors.some(err => err.path === 'name')).toBe(true);
  });

  it('should collect all errors when multiple fields invalid', async () => {
    const result = await baker.deserialize(MultiFieldErrorDto, { a: 1, b: 2, c: 3 });
    assertBakerIssueSet(result);
