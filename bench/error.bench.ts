...
const baker = new Baker({ stopAtFirstError: false });
...
// warm seal
baker.seal();
await baker.deserialize(BakerIssueSet, ERROR_ALL_FAIL);
...
group('error collection — 10 fields all invalid', () => {
  bench('baker', () => {
    const r = baker.deserialize(BakerIssueSet, ERROR_ALL_FAIL);
